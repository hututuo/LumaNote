    ...
    var body: some Scene {
        Settings {
            SettingsView(
                ...
                updateActions: appDelegate.updateActions
            )
            .frame(width: 460)
        }
        .commands {
...
final class AppDelegate: NSObject, NSApplicationDelegate {
    ...
    let noteStore = NoteStore(initialLoadMode: .deferred)
    lazy var clipboardStore = ClipboardStore(settings: settings)
    ...
    var updateActions: UpdateCheckingActions {
        UpdateCheckingActions(
            ...
            setAutomaticallyChecksForUpdates: { [weak self] isEnabled in
                self?.setAutomaticallyChecksForUpdates(isEnabled)
            }
        )
    }
