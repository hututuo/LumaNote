...
| `app/QuietNote/scripts/build-app.sh` | 2026-06-07 10:18 | 2026-06-11 00:48 | 构建脚本 | 编译、打包、ad-hoc 签名并生成 `build/LumaNote.app` | 不建议删除 | Sparkle 测试更新 | 会复制 `AppIcon.icns` 和 Sparkle.framework 到 app bundle，按由内到外顺序 ad-hoc 签 Sparkle 嵌套代码，外层 app 使用 `--requirements '=designated => identifier "com.hututuo.lumanote"'` 并严格验证 |
...
| `app/QuietNote/support/Info.plist` | 2026-06-07 10:19 | 2026-06-11 00:48 | app 配置模板 | `LumaNote.app` 的 bundle metadata 模板 | 不建议删除 | Sparkle 测试更新 | `CFBundleName`/`CFBundleDisplayName` 为 LumaNote，`LSUIElement=false`，使 app 出现在 Dock/任务栏；测试分支新增 `SUFeedURL` 和 Sparkle `SUPublicEDKey` |
