...
| `appcast-test/appcast.xml` | 2026-06-11 00:42 | 2026-06-11 00:42 | Sparkle 测试 feed | `test/ad-hoc-permission-update` 分支的 Sparkle 更新源 | 可替换 | Sparkle 测试更新包 | 当前指向 `0.1.1` / build `2`，enclosure 使用 EdDSA 签名 |
...
| `app/QuietNote/support/Info.plist` | 2026-06-07 10:19 | 2026-06-11 00:50 | app 配置模板 | `LumaNote.app` 的 bundle metadata 模板 | 不建议删除 | Sparkle 测试更新包 | `CFBundleName`/`CFBundleDisplayName` 为 LumaNote，`LSUIElement=false`，使 app 出现在 Dock/任务栏；测试分支新增 `SUFeedURL` 和 Sparkle `SUPublicEDKey`；测试更新版本为 `0.1.1` / build `2` |
...
| `app/QuietNote/Sources/AppText.swift` | 2026-06-07 17:30 | 2026-06-08 22:35 | 应用文案 | App 内中文/English 文案表 | 不建议删除 | 开机自启设置 | 设置文案区分“便签透明度”和“磨砂强度/玻璃质感”，用于设置页、更多菜单、剪切板库和动作菜单；新增切换便签文件、打开新文件、最近打开、开机自启和错误提示文案 |
...
| `app/QuietNote/Sources/SettingsView.swift` | 2026-06-07 10:05 | 2026-06-11 00:34 | 设置窗口 | app 设置表单 | 不建议删除 | ad-hoc permission update test | 新增中文/English 分段选择；快捷键拆成快捷呼出、快捷关闭、打开剪切板库；透明度和磨砂强度下方有简短说明；完整设置页新增开机自启开关、权限请求入口和失败提示 |
...
| `app/QuietNote/Sources/MoreMenuView.swift` | 2026-06-07 10:05 | 2026-06-11 00:34 | 紧凑设置菜单 | 便签内更多菜单和快捷键设置 sheet | 不建议删除 | ad-hoc permission update test | 菜单内容保持紧凑，背景由主窗口内玻璃浮层容器提供；顶部显示“玻璃质感”和模糊/泛白/质感说明，小窗口内可滚动；快捷键拆成呼出/关闭/剪切板库；新增开机自启开关、权限请求入口和失败提示 |
