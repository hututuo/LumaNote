...
| `appcast-test/appcast.xml` | 2026-06-11 00:42 | 2026-06-11 00:52 | Sparkle 测试 feed | `test/ad-hoc-permission-update` 分支的 Sparkle 更新源 | 可替换 | Sparkle 测试更新包 | 当前指向 `0.1.2` / build `3`，enclosure 使用 EdDSA 签名 |
...
| `appcast-test/LumaNote-0.1.1-macos-arm64.md` | 2026-06-11 00:41 | 2026-06-11 00:41 | Sparkle 更新说明 | Sparkle appcast 内嵌 release notes 来源 | 可替换 | Sparkle 测试更新包 | 说明本次用于测试 ad-hoc 签名 + Sparkle 更新后的权限保留 |
| `appcast-test/LumaNote-0.1.2-macos-arm64.zip` | 2026-06-11 00:46 | 2026-06-11 00:46 | Sparkle 测试更新包 | 供已安装的 `0.1.1` / build `2` 拉取更新 | 可替换 | Sparkle 可见更新入口测试 | 包内 app 为 ad-hoc signed，designated requirement 为 `identifier "com.hututuo.lumanote"`；新增可见“检查更新”入口后发布为 build `3` |
| `appcast-test/LumaNote-0.1.2-macos-arm64.md` | 2026-06-11 00:46 | 2026-06-11 00:46 | Sparkle 更新说明 | `0.1.2` appcast 内嵌 release notes 来源 | 可替换 | Sparkle 可见更新入口测试 | 说明本次用于测试可见更新入口和权限保留 |
| `appcast-test/LumaNote3-2.delta` | 2026-06-11 00:47 | 2026-06-11 00:47 | Sparkle delta 更新包 | 从 build `2` 到 build `3` 的差分更新 | 可替换 | Sparkle 可见更新入口测试 | 由 `generate_appcast` 自动生成；appcast 中引用了该文件，必须随 feed 提交 |
...
| `app/QuietNote/scripts/build-dmg.sh` | 2026-06-11 00:02 | 2026-06-11 00:28 | DMG 构建脚本 | 将已签名 `LumaNote.app`、`/Applications` 快捷方式和自定义玻璃背景打包为拖拽安装 DMG | 不建议删除 | GitHub DMG 下载流程测试 | 输出 `build/LumaNote-<version>-macos-arm64.dmg`，设置 Finder 图标布局/背景图，并执行 `hdiutil verify` |
...
| `app/QuietNote/support/Info.plist` | 2026-06-07 10:19 | 2026-06-11 00:52 | app 配置模板 | `LumaNote.app` 的 bundle metadata 模板 | 不建议删除 | Sparkle 测试更新包 | `CFBundleName`/`CFBundleDisplayName` 为 LumaNote，`LSUIElement=false`，使 app 出现在 Dock/任务栏；测试分支新增 `SUFeedURL` 和 Sparkle `SUPublicEDKey`；测试更新版本为 `0.1.2` / build `3` |
...
| `app/QuietNote/Sources/AppText.swift` | 2026-06-07 17:30 | 2026-06-11 00:46 | 应用文案 | App 内中文/English 文案表 | 不建议删除 | Sparkle 可见更新入口测试 | 设置文案区分“便签透明度”和“磨砂强度/玻璃质感”，用于设置页、更多菜单、剪切板库和动作菜单；新增切换便签文件、打开新文件、最近打开、开机自启、权限和检查更新文案 |
...
| `app/QuietNote/Sources/UpdateCheckButtonView.swift` | 2026-06-11 00:46 | 2026-06-11 00:46 | 更新测试 UI | 在更多菜单和设置页提供可见“检查更新”按钮 | 不建议删除 | Sparkle 可见更新入口测试 | 通过 `NSApp.sendAction` 调用 AppDelegate 的 Sparkle updater，避免用户只能从 macOS 顶部菜单或状态栏找更新入口 |
...
| `app/QuietNote/Sources/SettingsView.swift` | 2026-06-07 10:05 | 2026-06-11 00:46 | 设置窗口 | app 设置表单 | 不建议删除 | Sparkle 可见更新入口测试 | 新增中文/English 分段选择；快捷键拆成快捷呼出、快捷关闭、打开剪切板库；透明度和磨砂强度下方有简短说明；完整设置页新增开机自启开关、权限请求入口、检查更新入口和失败提示 |
...
| `app/QuietNote/Sources/MoreMenuView.swift` | 2026-06-07 10:05 | 2026-06-11 00:46 | 紧凑设置菜单 | 便签内更多菜单和快捷键设置 sheet | 不建议删除 | Sparkle 可见更新入口测试 | 菜单内容保持紧凑，背景由主窗口内玻璃浮层容器提供；顶部显示“玻璃质感”和模糊/泛白/质感说明，小窗口内可滚动；快捷键拆成呼出/关闭/剪切板库；新增开机自启开关、权限请求入口、检查更新入口和失败提示 |
...
| `app/QuietNote/Sources/QuietNoteApp.swift` | 2026-06-07 10:05 | 2026-06-11 00:48 | 应用入口 | 使用 `.regular` 应用模式并保留 reopen 显示窗口行为 | 不建议删除 | Sparkle 测试更新 | 修复进程存在但窗口不见、以及不在 Dock/任务栏显示的问题；状态栏菜单残留 QuietNote 文案改为 LumaNote；测试分支初始化 `SPUStandardUpdaterController` 并在 App 菜单/状态栏菜单提供 Check for Updates 入口 |
