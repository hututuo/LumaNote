...
- The bottom rail is an overlay above the Markdown editor rather than a separate layout row. On macOS 26+, it follows Apple's Liquid Glass API shape: the rail uses `glassEffect(.clear.interactive(), in:)`, rail controls sit in a `GlassEffectContainer`, and icon buttons use the built-in `.buttonStyle(.glass(.clear))`. Avoid adding white fog, tint overlays, or manual frosted-material layers on top of this clear glass path. Older macOS versions fall back to a very low-opacity `ultraThinMaterial` stack.
