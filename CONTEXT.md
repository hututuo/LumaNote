...
- The bottom rail is an overlay above the Markdown editor rather than a separate layout row. On macOS 26+, its background uses SwiftUI's real untinted `glassEffect(.regular.interactive(), in:)` Liquid Glass API, with only a hairline highlight and glass-effect buttons so note text can visually pass underneath the toolbar while the controls remain readable. Older macOS versions fall back to a very low-opacity `ultraThinMaterial` stack.
