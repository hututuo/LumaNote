...
- The bottom rail is an overlay above the Markdown editor rather than a separate layout row. Its material now follows the same glass-shell recipe as the top island: regular material, subtle white highlight, gradient border, and shadow strength all tied to `max(0.05, settings.noteOpacity)`. The rail shell becomes transparent with the note while keeping the icon content readable.
