...
- The bottom rail is an overlay above the Markdown editor rather than a separate layout row. On macOS 26+, it follows Apple's Liquid Glass API shape with stable clear glass: the rail uses `glassEffect(.clear, in:)`, rail controls sit in a `GlassEffectContainer`, and icon buttons use explicit `glassEffect(.clear, in:)` backgrounds. Avoid `.interactive()` or context-driven glass button styles on this rail because the system can change their inactive-window appearance when focus leaves LumaNote. Avoid adding white fog, tint overlays, or manual frosted-material layers on top of this clear glass path. Older macOS versions fall back to a very low-opacity `ultraThinMaterial` stack.
