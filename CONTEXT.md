...
- The bottom rail is an overlay above the Markdown editor rather than a separate layout row. On macOS 26+, its background uses SwiftUI's real `glassEffect(.regular.tint(...).interactive(), in:)` Liquid Glass API with near-clear tint/highlight so note text can visually pass underneath the toolbar while the controls remain readable. Older macOS versions fall back to the lighter `ultraThinMaterial` stack.
