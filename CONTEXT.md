...
- The island's title/detected value text and adjacent symbols should be stronger than the glass shell, not faded. Treat "20%/35% stronger" as increased visual weight from a full-strength baseline, not as 20%/35% opacity. Title mode and detection mode both use dark ink text/icons with a subtle white highlight, so the current note title and extracted clipboard values stay readable on the light glass island over bright Markdown or desktop content.
...
- Detection mode must read as an active clipboard extraction, not just a renamed title pill. Its capsule keeps the stronger theme-color border and subtle internal glass glow while showing the first extracted value. When the user clicks the extracted value area, the actions panel is an in-window overlay, not SwiftUI/macOS `.popover`, so there is no extra system material behind it. It should use one unified rounded glass panel and show a concise type summary such as `提取到：电话、地址` at the top; avoid per-row material cards inside that panel because they can look like mismatched panels. Extracted values in this panel should have a subtle theme-color outline so the specific copied text is easy to spot. Title mode and collapsed handles should not inherit the highlighted capsule state.
