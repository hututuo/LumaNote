...
- The bottom rail is an overlay above the Markdown editor rather than a separate layout row. Its background uses a lighter liquid-glass-style `ultraThinMaterial` stack so note text can visually pass underneath the toolbar while the controls remain readable.
