...
| Bold | `**bold**`, `__bold__` | Supported | Simple single-line bold styling. Deeply nested emphasis is not fully parsed. |
| Italic | `*italic*`, `_italic_` | Supported | Simple single-line italic styling. |
| Bold italic | `***text***`, `___text___` | Supported | Styled as bold text with italic slant. |
...
| Links | `[Apple](https://apple.com)`, `[Apple](https://apple.com "title")` | Supported | Styled as link-colored underlined text. The editor does not currently open links on click. URLs containing unmatched `)` may still be imperfect. |
...
| Fenced code blocks | <code>```swift</code>, `~~~` | Partial | Backtick and tilde fences are styled in monospace. Language-specific syntax highlighting is not implemented. |
